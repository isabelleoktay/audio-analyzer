...
const AudioFeaturesGraph = ({
  ...
  const [highlightsLength, setHighlightsLength] = useState(null);
  ...
  const drawData = (context, width, height, padding) => {
    data.forEach((values) => {
      ...
      lineData.forEach((value, index) => {
        const x =
          padding + (index / (lineData.length - 1)) * (width - 2 * padding);
        const y =
          height -
          padding -
  ...
  const drawHighlights = (context, width, height, padding) => {
    highlightedSections.forEach((section) => {
      ...
      const startX =
        padding + (start / highlightsLength) * (width - 3 * padding);
      const endX = padding + (end / highlightsLength) * (width - 3 * padding);
  ...
  useEffect(() => {
    setHighlightsLength(data[0].data.length);
  }, [data]);

...
const AudioFeaturesGraph = ({
  ...
  const lineCanvasRefs = useRef(datasets.map(() => React.createRef())); // Initialize for each dataset
  ...
  useEffect(() => {
    if (overlayCanvasRef.current) {
      ...
      const padding = 40;
      const width = overlayCanvasRef.current.clientWidth;
      const height = overlayCanvasRef.current.clientHeight;

      overlayContext.clearRect(0, 0, width, height);
      drawHighlights(overlayContext, width, height, padding);
    }
  }, [highlightedSections]);

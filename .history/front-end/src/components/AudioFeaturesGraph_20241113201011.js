...
const AudioFeaturesGraph = ({
  ...
  const drawData = (context, data, baseWidth, baseHeight, padding, color) => {
    ...
    data.forEach((value, index) => {
      const x =
        padding + (index / (data.length - 1)) * (baseWidth - 2 * padding);
      const y =
        baseHeight -
        padding -
        ((value - minY) / (maxY - minY)) * (baseHeight - 2 * padding);
      if (index === 0) {
        context.moveTo(x, y);
      } else {
        context.lineTo(x, y);
  ...
  const drawLineData = () => {
    ...
    // Use `baseCanvasRef` width and height for consistent scaling
    const baseCanvas = baseCanvasRef.current;
    const baseWidth = baseCanvas.width;
    const baseHeight = baseCanvas.height;

    datasets.forEach((dataset, idx) => {
      const canvas = lineCanvasRefs.current[idx];
      if (canvas) {
        const context = canvas.getContext("2d");
        ...
        // Draw data using base canvas dimensions
